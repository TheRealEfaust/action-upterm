...
export async function run() {
  try {
    ...
    if (process.platform == 'linux') {
      await execShellCommand(`curl -sL https://github.com/owenthereal/upterm/releases/latest/download/upterm_linux_amd64.tar.gz | tar zxvf - -C /tmp upterm`);
      await execShellCommand('if ! command -v tmux &>/dev/null; then yum install -y tmux; fi');
    } else {
